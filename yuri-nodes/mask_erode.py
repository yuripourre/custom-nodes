...
class MaskErode:
    ...
    def erode_mask(self, mask, iterations, kernel_size):
        # Convert to numpy
        ...
        for b in range(batch_size):
            ...
            for _ in range(iterations):
                eroded = ndimage.binary_erosion(eroded, structure=structure).astype(np.float32)
            
            # Convert back to float32 mask format
            result_mask = eroded.astype(np.float32)

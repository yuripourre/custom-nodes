...
class AlphaEdgeErode:
    ...
    def erode_alpha_edges(self, image, iterations, kernel_size, antialias=True):
        ...
        for b in range(batch_size):
            ...
            # Apply erosion to the entire alpha channel
            working_alpha = binary_alpha.copy()
            for _ in range(iterations):
                working_alpha = ndimage.binary_erosion(working_alpha, structure=structure).astype(np.float32)
            
            # Blend: use eroded alpha in edge regions, keep original elsewhere
